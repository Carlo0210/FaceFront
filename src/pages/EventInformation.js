...
const EventList = () => {
  ...
  useEffect(() => {
    async function fetchEvents() {
      try {
        const response = await axios.get('https://faceback.onrender.com/events');
  ...
  const handleCopyLink = (eventId, eventTitle, eventDate) => {
    // Convert the event date to "Month day, year" format
      ...
      <Row className="align-items-center">
        <Col md={{ offset: 1 }}>
          <Button
            ...
            style={{ backgroundColor: 'green', color: 'white', borderColor: '#fff' }}
          >
